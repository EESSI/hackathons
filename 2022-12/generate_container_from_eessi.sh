...
-b, --base		Optional: base image to use (detected: "$baseimage")
...
#lets figure this out now so we can print what we find in -h output
...
then
	baseimage="${ID}linux:${VERSION_ID%%.[0-9]}"
elif [ "$ID" = "rhel" ]
then
	baseimage="redhat:ubi${VERSION_ID%%.[0-9]}"
elif [ "$ID" = "almalinux" ]
then
	baseimage="${ID}:${VERSION_ID}"
else
	baseimage=""
fi
#be sure to pick an image that already includes tar
...
tar zcf $LOCAL/tarball.tgz --exclude "var/cache" --exclude "var/db" --exclude "var/tmp" --exclude "usr/share/doc" --exclude "*.log" --exclude "*.cvmfscatalog" $to_copy 2>/dev/null
...
then
	...
	echo
	msg "Building failed, check if -b base image exists on hub.docker.com"
	echo
fi
